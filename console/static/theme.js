...
(() => {
  ...
  const SAYS = [
    ...
    '上一次笑出声是什么时候？', '手机里有没有三个月没打开的应用？',
    '要不要给家里打个电话？', '今晚几点睡才算够本？',
    '桌上那杯水是不是放太久了？', '要不要把明天的待办先写下来？',
    '最近有没有从头到尾听完一张专辑？', '给窗边的植物转个方向吧？',
    '要不要出去走十分钟？', '你有多久没用纸笔记东西了？',
    '冰箱里那个还剩一半的还能吃吗？', '要不要把椅子调高一点点？',
    '今天有没有夸过谁？', '要不要给旧照片建个相册？',
    '这首曲子今天听第几遍了？', '有没有一句话想写给以后的自己？',
    '要不要把手机翻过来放一会儿？', '午饭吃到七分饱了没？',
    '今天看到的第一个笑话是什么？', '要不要把桌面收拾出一块空地？',
    '楼下那家新开的店你尝过了吗？', '有没有一本书读到一半就搁下了？',
    '要不要现在就伸个懒腰？', '今天的风是从哪边吹过来的？',
    '耳机线是不是又缠成一团了？', '今天想换条路走吗？',
    '有没有想见却很久没联系的人？', '要不要给自己买束花？',
    '上次看日落是什么时候？', '今天喝够水了吗？',
    '要不要把闹钟往后调十分钟？', '衣柜里有没有一年没穿的衣服？',
    '有没有一句话一直没说出口？', '要不要学着做一道新菜？',
    '今天有没有走够六千步？', '要不要给手机换个壁纸？',
    '最近有没有抬头看过星星？', '要不要把窗子开一会儿？',
    '有没有想重看一遍的老电影？', '今天的天气适合出门吗？',
    '要不要记下今天最开心的一件事？', '有没有想吃很久的店？',
    '要不要给明天的自己留张小纸条？', '今天有没有遇到有趣的人？',
    '要不要把常听的歌换个顺序？', '有没有想养却没养的植物？',
    '要不要试试十分钟不看屏幕？', '今天有没有好好吃早饭？',
    '有没有一件小事让你笑了？', '要不要把椅子挪到有光的地方？',
    '最近有没有写过超过三行的话？', '要不要给水杯换个位置？',
    '今天想听点什么样的音乐？', '有没有想去却一直没去的地方？',
    '要不要把明天的衣服先拿出来？', '今天有没有对自己好一点？',
    '要不要现在就关掉一个通知？', '有没有想推荐给别人的东西？',
    '要不要数数今天叹了几次气？', '今天有没有走得比昨天远一点？',
    '有没有想学的奇怪小技能？', '要不要把窗户擦一擦？',
    '今天想早点睡还是晚点睡？',
  ];
  ...
  const TAILS = [
    ...
    '（￣▽￣）', '(๑´ㅂ`๑)', '嗷呜', '喵呜', '嗷喵？', '咪呜', '嗷嗷', '呜喵',
    '咪嗷', '喵嗷', '咕噜咕噜', '呼噜呼噜', '嗷呜～', '咪呜～',
  ];
